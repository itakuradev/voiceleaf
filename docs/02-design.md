...
```text
┌─────────────────────────────┐
│      🌿 独り言要約 🌿         │ ← title.png（画像）・左右中央
│   話すだけで、頭の中をスッキリ整理。 │ ← サブコピー（ミュート色）・中央
│                             │
│ ┌─────────────────────────┐ │
...
│                             │
│   [思いついたことを          │ ← please-talke-me.png（画像）
│    自由に話してくださいね]     │
│                             │
│   ┌───────────────────┐     │
...
| ファイル | 使用画面 | 配置 |
| --- | --- | --- |
| `title.png`（「独り言要約」＋左右の葉飾り） | SCR-01 | 画面上部・左右中央 |
| `deco-bottom-left.png`（植物） | SCR-01 | 左下 |
| `deco-bottom-right.png`（コーヒー・本） | SCR-01 | 右下 |
| `leaf-small.png` | SCR-02 | 見出し上部 |
| `please-talke-me.png`（「思いついたことを / 自由に話してくださいね」） | SCR-02 | 録音ボタンの下 |
| `deco-note.png`（ノート・ペン） | SCR-03 | ヘッダー下 |
| `deco-branch.png`（枝） | SCR-03 | 要約ボックス右下 |

すべて背景透過のPNGとし、不透明領域でトリミングしたうえで長辺600pxに揃える。
表示サイズは画面コード側で指定するため、画像自体に余白を持たせない。

`title.png` と `please-talke-me.png` は、文字を**テキストではなく画像として配置する**。
デザイン画像の明朝体・手書き風の字形はフォントで再現できないため、画像で持つ（12章 #19）。
このため当該文言だけは 8.2 のタイポグラフィの対象外となる。
...
| 18 | 装飾イラスト | 背景透過PNGを別途用意し、不透明領域でトリミングして長辺600pxに統一 | 当初はデザイン画像から矩形で切り出したが、画面のベゼルが写り込むうえ背景色差で矩形の輪郭が見えたため。透過画像に差し替えたことで #12（背景テクスチャを再現しない）とも整合する |
| 19 | アプリタイトルと録音画面のヒント文 | テキストではなく画像（`title.png` / `please-talke-me.png`）で配置し、いずれも左右中央に置く | デザイン画像の明朝体・手書き風の字形は標準フォントでは再現できないため。文言が固定で状態によって変化しないため、画像化の弊害が小さい |
