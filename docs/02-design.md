...
| 分類 | 採用技術 | 選定理由 |
| --- | --- | --- |
| フレームワーク | React Native / Expo (SDK 54系) | 要件定義の制約 |
...
│  ├─ _layout.tsx              # Stack定義 / Provider配置 / フォント読み込み
...
| `processing` | 「要約しています」＋スピナー。ボタン非表示 | **全操作を受け付けない**（Androidバックキーも無効化） |
| `saved` / `failed` | — | 自動でトップへ遷移 |

> `processing` 中はハードウェアバックキーを `BackHandler` で握りつぶし、FN-08「AI処理中は追加操作を受け付けない」を満たす。
...
| 戻る操作 | ヘッダーに戻るボタンは置かず、Androidバックキーのみで中断（デザイン画像に戻るボタンが無いため） |
| processing表示 | 見出しを「要約しています」に、マイクアイコンをスピナーに、下部ボタンを非表示に切り替え。同心円装飾は維持 |
...
デザイン画像は**明朝体（セリフ系）**が使われている。Androidの標準日本語フォントはゴシック（Noto Sans CJK）のため、そのままでは再現できない。

**方針**：Google Fonts の `Noto Serif JP` を `@expo-google-fonts/noto-serif-jp` で読み込み、`app/_layout.tsx` でフォント読込完了までスプラッシュを保持する。
