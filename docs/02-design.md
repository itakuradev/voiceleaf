...
`please-talke-me.png` は、録音画面のヒント文を**テキストではなく画像として配置する**。
デザイン画像の手書き風の字形はフォントで再現できないため、画像で持つ（12章 #19）。
...
| 19 | 録音画面のヒント文 | テキストではなく画像（`please-talke-me.png`）で配置 | デザイン画像の手書き風の字形は標準フォントでは再現できないため。文言が固定で状態によって変化しないため、画像化の弊害が小さい |
